/**
  ...
  6/20/2017
  David Grullon - I have downloaded this from Crim's library
  on GitHub and have modified it a little to suit my needs.
  Unfortunately, I do not have a log of what I changed but
  know that it works for my Mega2560. I have done my best to
  add additional comments for I have done.
...
{
	// Create the variables we need for the function to work
	int ecuByte[3];		// Stores the ECU's information
	int initializer;	// Should store our 0x10 value
	int counter = 0;	// Count how many reads done

	// Verify if we have set the Serial. If not, go home and set it asshole >:-O
	if (_consultSerial == NULL) { return false; }

	writeEcu(0xFF);
	writeEcu(0xFF);
	writeEcu(0xEF);

	// This will read our entire buffer. Before it just read the first
	// input in the buffer and that was 0x00. Now, we will read the entire
	// buffer and read it's last variable, which should be 0x10 (hopefully).
	// The protocol sheet says that it sometimes takes two cycles to work.
	while (_consultSerial->available()>0)
	{
		readEcu(&ecuByte[counter]);
		counter++;
	}

	// Now that we read the buffer, let us check if we received a 0x10.
	// You can do a for loop and read through the entire buffer but we
	// we know that we will receive three readings and it's the third
	// one that should be 0x10. So I made it read only that variable.
	initializer = ecuByte[counter - 1];
...
void Consult::writeEcu(byte v)
{
  ...
  // David - I added this delay in here because I find that
  // the Arduino's speed of 16 MHz is WAY faster than the
  // 9600 baud so if we send a command to read the data,
  // it will be reading nothing because the command will be sent
  // before the ECU has even had a chance to fully respond.
  // NOTE: I DID NOT TIME THIS. BUT I DO KNOW THAT 1 MS DID NOT WORK
  // FOR WHAT I NEEDED. I AM CONSCIOUS OF THE FACT THAT 2 MS IS WASTING
  // A LOT OF TIME. 
}
